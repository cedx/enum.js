...
# Tests the features of the `Enum` interface.
describe "Enum", ->
	SampleEnum = Enum
		zero: false
		one: 1
	...
	describe "entries()", ->
		it "should return the pairs of names and values", ->
			entries = SampleEnum.entries()
			equal entries.size, 4

			[zero, one, two, three] = entries
	...
	describe "has()", ->
		it "should return `false` for unknown names", ->
			ok not SampleEnum.has "foo"
			ok not SampleEnum.has "TWO"
	...
	describe "hasValue()", ->
		it "should return `false` for unknown values", ->
			ok not SampleEnum.hasValue 0
			ok not SampleEnum.hasValue "two"
			ok not SampleEnum.hasValue 3.5
