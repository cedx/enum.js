...
test("createEnum()", async t => {
	await t.test("should create types that are immutable", () => {
		assert.ok(Object.isFrozen(SampleEnum));
	});

	await t.test("should create types having the `EnumMixin` mixin", () => {
...
test("createEnum().assert()", async t => {
	await t.test("should return the specified value if it is a known one", () => {
	...
	await t.test("should throw an exception if it is an unknown value", () => {
...
test("createEnum().coerce()", async t => {
	await t.test("should return the specified value if it is a known one", () => {
	...
	await t.test("should return the default value if it is an unknown one", () => {
...
test("createEnum().getEntries()", async t => {
	await t.test("should return the pairs of names and values", () => {
...
test("createEnum().getIndex()", async t => {
	await t.test("should return `-1` for unknown values", () => {
		assert.equal(SampleEnum.getIndex(0), -1);
		assert.equal(SampleEnum.getIndex("two"), -1);
		assert.equal(SampleEnum.getIndex(3.1), -1);
	});

	await t.test("should return the index for known values", () => {
...
test("createEnum().getName()", async t => {
	await t.test("should return an empty string for unknown values", () => {
	...
	await t.test("should return the name for known values", () => {
...
test("createEnum().getNames()", async t => {
	await t.test("should return the names of the enumerable properties", () => {
		assert.deepEqual(SampleEnum.getNames(), ["zero", "one", "two", "three"]);
	});
});

test("createEnum().getValues()", async t => {
	await t.test("should return the values of the enumerable properties", () => {
		assert.deepEqual(SampleEnum.getValues(), [false, 1, "TWO", 3.0]);
	});
});

test("createEnum().isDefined()", async t => {
	await t.test("should return `false` for unknown values", () => {
		assert.ok(!SampleEnum.isDefined(0));
		assert.ok(!SampleEnum.isDefined("two"));
		assert.ok(!SampleEnum.isDefined(3.1));
	});

	await t.test("should return `true` for known values", () => {
